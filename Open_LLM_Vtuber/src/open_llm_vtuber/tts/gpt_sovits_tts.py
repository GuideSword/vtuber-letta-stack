...
class TTSEngine(TTSInterface):
    ...
    def generate_audio(self, text, file_name_no_ext=None):
        ...
        if not self.api_url.endswith('/tts'):
            api_url = self.api_url.rstrip('/') + '/tts'
        else:
            api_url = self.api_url
        
        # Prepare the data for the POST request
        # 使用GPT-Sovits API期望的参数名称
        data = {
            "text": cleaned_text,
        ...
        logger.info(f"Sending GPT-Sovits request to: {api_url}")
        ...
        try:
            ...
            logger.info(f"Response content: {response.text[:200]}...")  # 只记录前200个字符
            ...
            logger.critical(f"Error: Exception occurred while sending request: {e}")
            return None
